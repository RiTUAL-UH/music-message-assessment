...
def metric_penalty(a, b):
    return abs(a - b)


def softing_label(label, any_class, class_num=3):
    numerator = math.exp(-(metric_penalty(label, any_class)))
    denominator = sum([math.exp(-(metric_penalty(label, i))) for i in range(class_num)])
    return numerator / denominator


def digits_to_soft_labels(label):
    return torch.tensor([softing_label(label, i) for i in range(3)])


def softing_label_list(label_list):
    return torch.stack([digits_to_soft_labels(x) for x in label_list])
...
logger = ExperimentLog(args.log_save_dir + "record_soft_10CV.txt")
...
for each_seed in random_seeds:
    ...
    for counting in range(10):
        ...
        def loss_function(prediction, target):
            return F.kl_div(F.log_softmax(prediction), target, reduction="sum")

        # logger('this is test')

        # logger('anything I want')
        ...
        for epoch in range(args.training_epochs):
            ...
            if args.dev_run != True:
                ...
                with tqdm(train_loader, unit=" batch") as tepoch:
                    for idx, (text, emo, target) in enumerate(tepoch):
                        ...
                        vio_target = softing_label_list(get_column(target, 0)).to(
                            to_device
                        )
                        sub_target = softing_label_list(get_column(target, 1)).to(
                            to_device
                        )
                        sex_target = softing_label_list(get_column(target, 2)).to(
                            to_device
                        )
                        con_target = softing_label_list(get_column(target, 3)).to(
                            to_device
                        )
                        pos_target = softing_label_list(get_column(target, 4)).to(
                            to_device
                        )

                        #                     prediction = model(text, batch_size = len(target))
                        prediction = model(text, emo, batch_size=len(target))

                        vio_loss = loss_function(prediction[0], vio_target)
                        sub_loss = loss_function(prediction[1], sub_target)
                        sex_loss = loss_function(prediction[2], sex_target)
                        con_loss = loss_function(prediction[3], con_target)
                        pos_loss = loss_function(prediction[4], pos_target)

                        loss = vio_loss + sub_loss + sex_loss + con_loss + pos_loss
            ...
            with tqdm(dev_loader, unit=" batch") as vepoch:
                for idx, (text, emo, target) in enumerate(vepoch):
                    ...
                    vio_target = softing_label_list(get_column(target, 0)).to(to_device)
                    sub_target = softing_label_list(get_column(target, 1)).to(to_device)
                    sex_target = softing_label_list(get_column(target, 2)).to(to_device)
                    con_target = softing_label_list(get_column(target, 3)).to(to_device)
                    pos_target = softing_label_list(get_column(target, 4)).to(to_device)
                    ...
                    vio_pred_digits = [torch.argmax(x).item() for x in prediction[0]]
                    sub_pred_digits = [torch.argmax(x).item() for x in prediction[1]]
                    sex_pred_digits = [torch.argmax(x).item() for x in prediction[2]]
                    con_pred_digits = [torch.argmax(x).item() for x in prediction[3]]
                    pos_pred_digits = [torch.argmax(x).item() for x in prediction[4]]

                    val_loss = vio_loss + sub_loss + sex_loss + con_loss + pos_loss
                    ...
                    val_record = {
                        ...
                        "vio_target": get_column(target, 0).tolist(),
                        "sub_target": get_column(target, 1).tolist(),
                        "sex_target": get_column(target, 2).tolist(),
                        "con_target": get_column(target, 3).tolist(),
                        "pos_target": get_column(target, 4).tolist(),
                    }
                    # collect with a list
        ...
        for idx, (text, emo, target) in enumerate(tqdm(test_loader)):
            ...
            test_loss = vio_loss + sub_loss + sex_loss + con_loss + pos_loss
